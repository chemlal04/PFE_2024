...
<div class="overflow-x-auto shadow-md sm:rounded-lg">
    <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
        ...
        <tbody>
            ...
            <tr class="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700">
                ...
                <td  class="{{ $vol->statut === 'approved' ? 'text-white text-center bg-green-500 rounded w-20'  : ($vol->statut === 'rejected' ? 'text-white text-center bg-red-500 rounded w-20' : '') }}">{{ $vol->statut }}</td>
    ...
    <form method="POST" action="{{ route('statusUpdate', $vol->id) }}" style="display:inline;">
        ...
        <button type="submit" class="font-medium text-red-600 dark:text-red-500 hover:underline ml-4">Reject</button>
    </form>
</td>
